...
class NaiveReplayBuffer(ABC):
    ...
    def setup_dynamic_batch(self, strategy):
        ...
        num_steps = torch.tensor(num_steps, dtype=torch.int, device=torch.accelerator.current_accelerator())
        ...
        num_microbatches = torch.tensor(num_microbatches, dtype=torch.int, device=torch.accelerator.current_accelerator())
        ...
        for partitions in data_partitions:
            ...
            global_valid_sample_num = torch.tensor(
                valid_sample_num, dtype=torch.float, device=torch.accelerator.current_accelerator()
            )
