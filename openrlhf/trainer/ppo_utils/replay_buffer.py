...
class NaiveReplayBuffer(ABC):
    ...
    def __init__(
        ...
        self.target_device = f"{torch.accelerator.current_accelerator().type}:{torch.accelerator.current_device_index()}"
    ...
    def setup_dynamic_batch(self, strategy):
        ...
        num_microbatches = torch.tensor(num_microbatches, dtype=torch.int, device=torch.accelerator.current_device_index())
