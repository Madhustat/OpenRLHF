...
def resolve_vllm_sync_backend(backend=None):
    ...
    When ``backend`` is None (unset), auto-select NCCL/RCCL on CUDA/ROCm and gloo (CPU-staged)
    on every other accelerator, including Intel XPU. An explicitly configured backend is
    validated strictly: an impossible combination (e.g. 'nccl' without CUDA/ROCm) raises rather
