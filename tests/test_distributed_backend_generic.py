...
def test_two_rank_allreduce_crosses_processes(
    ...
    try:
        ...
        if timed_out:
            ...
            pytest.fail(
                "Distributed worker timed out "
                f"(device={device}, backend={backend}, pids={timed_out_pids})"
            )
        ...
        assert not worker_errors, (
            f"Distributed worker failures for device={device}, "
            f"backend={backend}:\n{worker_errors}"
        )
        ...
        assert not exit_failures, (
            f"Distributed workers exited unsuccessfully "
            f"(device={device}, backend={backend}): {exit_failures}"
        )

        assert set(results) == {0, 1}, (
            f"Expected results from ranks 0 and 1, got {results}"
        )

        for rank, result in results.items():
            assert result["matched"], (
                f"Rank {rank} ended with {result['value']}; expected "
                "[3.0, 3.0, 3.0, 3.0] after all_reduce(SUM)"
            )
