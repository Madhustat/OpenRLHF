...
ray_start() {
    ...
    rm -rf /tmp/ray
    sleep 4
    # XPU health gate — log device health before each case so a wedged device is
    # attributable to the prior case.
    if ONEAPI_DEVICE_SELECTOR=level_zero:0 timeout 30 "$VENV/bin/python" -c \
        "import torch;t=torch.ones(2,device='xpu');torch.xpu.synchronize();assert float(t.sum())==2.0" \
        >/dev/null 2>&1; then
        echo "[HEALTH] XPU OK"
    else
        echo "[HEALTH] WARNING: XPU not responsive (device may be wedged)"
    fi
...
run_rl_singlegpu_test "sg_grpo_kl" \
    ...
    --ds.zero_stage 2   # adam_offload dropped: host-RAM-bound box (ref model role), optimizer to VRAM
...
run_rl_singlegpu_test "sg_reinforce_baseline" \
    ...
    --rollout.n_samples_per_prompt 4 --rollout.batch_size 8 \
    --ds.zero_stage 2   # adam_offload dropped on purpose: this box is host-RAM-bound (31GB); keep optimizer on idle VRAM
