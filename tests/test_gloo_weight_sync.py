...
# The resolver reads only torch.version.cuda / .hip to decide nccl-vs-gloo, so patching those
# three lets us simulate CUDA / ROCm / XPU builds regardless of the box the tests run on.
# Weight sync uses gloo on every non-CUDA/ROCm accelerator, including Intel XPU: gloo does not
...
@pytest.mark.parametrize(
    ...
    [
        # auto-detect (unset): nccl on CUDA/ROCm, gloo everywhere else
        ("12.1", None, None, None, "nccl"),      # CUDA -> nccl
        (None, "6.0", None, None, "nccl"),       # ROCm -> nccl
        (None, None, "20250", None, "gloo"),     # Intel XPU -> gloo
        (None, None, None, None, "gloo"),        # nothing detected -> gloo
        # explicit values are honored where possible
        ("12.1", None, None, "nccl", "nccl"),
        (None, None, "20250", "gloo", "gloo"),   # explicit gloo on XPU
        ("12.1", None, None, "gloo", "gloo"),    # explicit gloo on CUDA
    ],
)
...
def test_resolve_backend_unknown_raises():
    p_cuda, p_hip, p_xpu = _patch_platform(cuda="12.1", hip=None)
