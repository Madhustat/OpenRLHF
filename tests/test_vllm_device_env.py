...
def _call(backend, platform_env_var, gpu_ids, noset=True, bundle_indices=None, num_gpus=1):
    ...
    with patch.dict("os.environ", clean_env, clear=True), patch(
        ...
        RolloutRayActor._configure_device_env(
            object.__new__(RolloutRayActor), backend, bundle_indices, num_gpus
        )
        import os
...
def test_bundle_indices_env_preserved():
    env = _call(
        backend="mp", platform_env_var="ZE_AFFINITY_MASK", gpu_ids=[0], bundle_indices=[4, 5], num_gpus=2
    )
